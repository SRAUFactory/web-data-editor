...
class WDEControllerTest extends  PHPUnit_Framework_TestCase {
    ...
    private function getExpectedHtml($dataView) {
        $expected = <<<EOF
<!DOCTYPE html>
<html lang="ja">
<head>
...
EOF;
        return str_replace("#####dataView#####", $dataView, $expected); 
    }
     ...
     */ 
    function testIndex() {
        $this->assertSame($this->getExpectedHtml(""), $this->controller->index());
    }
...
EOF;
        $this->assertSame($this->getExpectedHtml($dataView), $this->controller->upload());
    }
	...
	$expected = [
	    "header" => [
                'Content-Type: text/'. $_POST["fileType"],
		'Content-disposition: attachment; filename="'.$fileName. '"',
            ],
	    "body" => file_get_contents(dirname(__FILE__) . '/../data/'.$fileName),
        ];
        $this->assertSame($expected, $this->controller->download());
    }
}
